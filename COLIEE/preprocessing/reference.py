...
os.mkdir('./COLIEE/task1_test_2023/processed_new')
...
total = 0
for name in names:
    with open(f"./COLIEE/task1_test_2023/task1_test_files_2023/{name}", "r", encoding="utf-8") as f:
        t = f.read()
        idx_ = t.find("[1]")
        if idx_ != -1:
            t = t[idx_:]
    ...
    now_len = len(words)
    if len(words) < 512:
        with open(f"./COLIEE/task1_test_2023/processed/{name}", "r", encoding="utf-8") as fd:
            new_txt = fd.read()
            ...
            for ii, sl in enumerate(slt):
                ...
                if now_len > 512:
                    break
            txt = "".join(slt[:ii]) + txt
    with open(f"./COLIEE/task1_test_2023/processed_new/{name}", "w+", encoding="utf-8") as f:
        f.write(txt)
